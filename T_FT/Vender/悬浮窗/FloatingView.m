...
#define sleepAlpha  0.45           //隐藏到边缘时的背景alpha值
...
- (void)changBoundsabovePanPoint:(CGPoint)panPoint{
    ...
    if(panPoint.y < kk_HEIGHT/2 + STATUSVIEW_HEIGHT){
       
        [UIView animateWithDuration:animateDuration animations:^{
            if(panPoint.x <= kScreenWidth/2){
                     self.center = CGPointMake(kk_WIDTH/2, kk_HEIGHT/2 + STATUSVIEW_HEIGHT);
            }else{
                 self.center = CGPointMake(kScreenHeight-kk_WIDTH/2, kk_HEIGHT/2 + STATUSVIEW_HEIGHT);
            }
